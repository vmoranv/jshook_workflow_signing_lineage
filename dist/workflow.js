...
export default createWorkflow(workflowId, 'Signing Lineage')
    .description('Traces the full signing lineage from ciphertext back to plaintext: intercepts signed requests, locates the signing function via initiator stacks, extracts the parameter normalization chain, hooks intermediate transforms, and captures the complete plaintext → normalize → concat → hash/encrypt → inject pipeline.')
    ...
    const searchKeywords = String(ctx.getConfig(`${prefix}.searchKeywords`, 'sign,signature,encrypt,hmac,md5,sha,hash,token,digest,secret,key,iv,nonce'));
